	...
	<view class="gg" v-if="token">
		...
		<u-navbar :is-back="false" :border-bottom="false"
			...
			<view class="gg-map-slot-wrap u-font-xs u-m-l-20 u-p-l-10 u-p-r-10 u-p-t-5 u-p-b-5" @click="pickUpLocation">
				...
				<text
					class="u-p-l-10 u-p-r-10">{{ leaderAddressVo.takeName ? leaderAddressVo.takeName : '请设置提货点' }}</text>
		...
		<view class="gg-content">
					...
					<scroll-view scroll-x enable-flex class="gg-new-vip-sv">
						<view class="u-flex u-m-t-10">
							<view class="gg-new-vip-sv-item u-p-r-20" v-for="(item, index) in newPersonSkuInfoList"
								...
								<ListImgItem :src="item.imgUrl" width="200rpx" height="200rpx"
									:showLeft="item.skuType === 1" :showRight="false"
			...
			<view class="u-m-20 u-p-20 gg-border">
				...
				<view class="gg-category"><u-line-progress class="gg-category-progress" height="5" :show-percent="false"
						:percent="categorySVPercent"></u-line-progress></view>
			</view>
			...
			<block v-if="seckillSkuVoList.length > 0">
				...
				<view class="u-m-l-20 u-m-r-20 u-flex">
					<view class="u-flex u-flex-1">
						<view class="u-font-lg u-type-error">
							秒杀抢购：
							<text class="u-font-sm">{{ seckillTime.name }}场
								{{ seckillTime.startTime }}-{{ seckillTime.endTime }}</text>
						</view>
					</view>
				...
				<view class="u-m-20 u-p-20 gg-border">
					<scroll-view enable-flex scroll-x>
						<view class="u-flex u-p-r-20 u-p-t-20">
							<!-- 循环滚动内容 -->
							<view class="u-p-l-20 u-p-r-20 u-text-center" v-for="item in seckillSkuVoList"
								:key="item.skuId" @click="gotoProductItem(item.skuId)">
								<text class="u-font-sm u-m-b-5">{{ item.timeName }}</text>
								<u-image :src="item.imgUrl" border-radius="30rpx" width="200rpx"
			...
			<view class="u-p-20 u-m-20 gg-border" v-for="(item, index) in hotSkuList" :key="item.id"
				...
				<view class="u-m-b-10 u-m-l-20 u-m-r-20 u-flex gg-product-item">
					<ListImgItem :src="item.imgUrl" width="250rpx" height="250rpx" :showLeft="item.skuType === 1"
						:showRight="false" :showBottom="item.skuType === 0 && item.isNewPerson === 1"></ListImgItem>
