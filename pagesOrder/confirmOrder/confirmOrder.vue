		...
		<view class="gg-content">
			<scroll-view scroll-y="true" class="gg-cart-sv-container" v-if="getCartInfoList">
							...
							<view v-for="(cartInfoItem, idx) in cartInfoListItem.cartInfoList" :key="idx">
								...
								<view class="u-body-item u-flex u-col-between u-p-10"
									...
									<ListImgItem :lazyLoad="false" :src="cartInfoItem.imgUrl" width="200rpx"
										height="200rpx" :showLeft="cartInfoItem.skuType === 1" :showRight="false"
				...
				<u-card :padding="10" :show-head="false" v-if="getCartCouponInfoList.length > 0">
					<view slot="body">
						<view class="coupon">
							<view class="content u-m-b-20" v-for="couponInfoItem in getCartCouponInfoList"
								:key="couponInfoItem.id" :class="{ selected: couponInfoItem.selected }"
								...
								<view class="centre">
									...
									<view class="valid-date">
										过期时间：{{ dayjs(couponInfoItem.expireTime).format('YYYY-MM-DD') }}</view>
								</view>
								<view class="right">
									<block v-if="couponInfoItem.isSelect === 1"><u-tag text="可以使用" type="error" />
									</block>
									<block v-else><u-tag text="不能使用" type="info" /></block>

									<block v-if="couponInfoItem.isOptimal === 1"><u-tag text="最优推荐" type="error" />
									</block>
						...
						<view class="item u-p-l-20">
							<view class="u-font-sm u-line-1">
								...
								<view class="u-font-xs u-type-info">
									...
									<text
										class="u-font-xs u-type-info">￥{{ getCartPriceInfo.originalTotalAmount }}</text>
								</view>
							</view>
						</view>
					</view>
